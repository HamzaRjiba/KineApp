...
const EmploiKine = ({ kineId }) => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (storedData) {
        ...
        axios
          .get('http://192.168.1.10:8000/api/kine/' + userData.kineId + '/emp')
          .then((response) => {
  ...
  const sendRendezVous = () => {
    ...
    axios
      .post('http://192.168.1.10:8000/rendez-vous', rendezVousData)
      .then((response) => {
        alert('Rendez-vous créé avec succès', response.data);
